...
\end{frame}
%%%%%%
\subsection{Entornos Matem\'aticos}
...
\begin{frame}{Usando Im\'agenes}
  \begin{itemize}
    ...
    \item<3-> Se puede insertar archivos EPS, PNG, JPG y PDF. Si dispone de varias versiones de la imagen entonces se puede escribir la extensión.
  \end{itemize}
  \uncover<4->{
  \begin{tabular}{ll}
    \begin{minipage}{0.5\textwidth}
...
\end{frame}
%%%%%%
\subsection{Bases de datos de referencias bibliográficas}
\begin{frame}{Bases de datos de referencias bibliográficas}
  \begin{itemize}
    \item<1-> Una forma más organizada de trabajar, es construir una base de datos bibliográfica y emplear posteriormente
    el programa BibTex para construir nuestra sección de referencias.
    \item<2->La base de datos bibliográfica consiste en uno (ó varios) archivos de extensión .bib, que contienen registros  con toda la información bibliográfica.
    \item<3-> La sintaxis básica de un registro es:
    
    \texttt{\footnotesize\color{blue}@TipoDeRegistro\{Etiqueta,CampoA=\{Contenido\},CampoB=\{Contenido\},etc..\}}
    
    donde TipoDeRegistro denota qué clase de documento referenciamos, y Etiqueta va a ser la
    etiqueta que debemos emplear como argumento del comando \texttt{\color{blue}cite} para citar la referencia en el documento.
    \item<4-> Posteriormente, se debe especificar una serie de campos, algunos
...
\begin{frame}{Bases de datos de referencias bibliográficas}
  \begin{itemize}
    \item<1-> En el siguiente ejemplo se muestra el contenido de un archivo .bib
...
\begin{frame}{Bases de datos de referencias bibliográficas}
  \begin{itemize}
    \item<1-> Una vez construido el archivo .bib (que debe estar colocado en el directorio del documento), se colocan a lo largo del documento los diversos comandos
    \texttt{\color{blue}cite}.
    \item<2-> La sección de biliografía puede construirse automáticamente colocando
    en el lugar adecuado (generalmente, al final) los comandos:
    
    \texttt{\color{blue}bibliography\{NombreArchivo\}} y \texttt{\color{blue}bibliographystyle\{style\}}

    %\bibliography{NombreFichero} \bibliographystyle{style}
    \item<3->El primero tiene como argumento el nombre del archivo (sin incluir extensión) y se encarga de construir la lista de referencias bibliográficas.
    \item<4->El segundo, especifica el estilo bibliográfico a emplear para construir las referencias.
    \end{itemize}
  \end{frame}
  %%%%
  \begin{frame}{Bases de datos de referencias bibliográficas}
    \begin{itemize}
      \item<1-> Existen cuatro estilos
      estándar disponibles en LATEX:
      \begin{itemize}
        \item<2-> \texttt{\color{blue}unsrt}: Las entradas se numeran, y aparecen en la lista según el orden en el que son citadas a lo
        largo del documento
        \item<3-> \texttt{\color{blue}plain}: Análogo a unsrt, pero las entradas son ordenadas alfabéticamente. El orden es
        autor, después año, y por último título.
        \item<4-> \texttt{\color{blue}alpha}: Las referencias se identifican con el nombre de autor y año de publicación. Se ordenan por etiqueta, autor, año y título.
        \item<5-> \texttt{\color{blue}abbrv}: Como plain, pero con los nombres de autores, y revistas abreviados.        
      \end{itemize}
    \end{itemize}
  \end{frame}
  %%%%
  \begin{frame}{Bases de datos de referencias bibliográficas}
    \begin{itemize}
      \item<1-> La compilación del documento se efectúa entonces primero ejecutando latex archivo.tex,
      \item<2-> Posteriormente se ejecuta bibtex archivo (lo cual genera la lista de bibliografía en un archivo auxiliar)
      \item<3-> Finalmente se termina con una segunda compilación en latex del documento fuente (que incluye en el mismo la lista de referencias).
      \item<4-> El paquete \texttt{\color{blue}biblatex} permite la incorporación de bases de datos de referencias bibliográficas y extendiendo la funcionalidad de \LaTeX.
    \end{itemize}
  \end{frame}
\end{document}
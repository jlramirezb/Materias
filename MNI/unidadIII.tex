...
$$
\begin{array}{cc}
\includegraphics[scale=0.23]{lagrange.eps} & \includegraphics[scale=0.23]{errorlagrange.eps}\\
...
$$
\begin{array}{cc}
\includegraphics[scale=0.23]{spline.eps} & \includegraphics[scale=0.23]{errorspline.eps}\\
...
$$
\begin{array}{cc}
\includegraphics[scale=0.23]{splineder.eps} & \includegraphics[scale=0.23]{errorsplineder.eps}\\
\textcolor{red}{\frac{1}{1+x^2}} \text{ vs }\textcolor{blue}{S(x)} & |E(x)|=|f(x)-S(x)| 
\end{array}
$$
}
...
\frametitle{M\'inimos Cuadrados}
\begin{itemize}
  \item<1-> Desarrollando queda:
\begin{eqnarray}
 \nonumber E & = & \sum_{i=0}^My_i^2 - 2\underbrace{\sum_{i=0}^M\left(y_i\sum_{k=0}^na_kx_i^k\right)}_{A} + 
\underbrace{\sum_{i=0}^M\left(\sum_{k=0}^na_kx_i^k\right)^2}_{B}\\
\nonumber A & = & \sum_{i=0}^M\left(y_i\sum_{k=0}^na_kx_i^k\right) = \sum_{i=0}^M\sum_{k=0}^na_ky_ix_i^k = \\
\nonumber & = & \sum_{k=0}^n\sum_{i=0}^Ma_ky_ix_i^k = \sum_{k=0}^n\left(a_k\sum_{i=0}^My_ix_i^k\right)\\
\nonumber \dfrac{\partial A}{\partial a_j} & = & \sum_{i=0}^My_ix_i^j
\end{eqnarray}
\end{itemize}}
%%%%
\frame{
\frametitle{M\'inimos Cuadrados}
\begin{itemize}
  \item<1-> Desarrollando $B$
\begin{eqnarray}
  \nonumber B & = & \sum_{i=0}^M\left(\sum_{k=0}^na_kx_i^k\right)^2 = 
  \sum_{i=0}^M\sum_{j=0}^n\sum_{k=0}^na_jx_i^ja_kx_i^k\\
  \nonumber& =&  \sum_{i=0}^M\sum_{j=0}^n\sum_{k=0}^na_ja_kx_i^{j+k} = 
  \sum_{i=0}^M\left(\sum_{k=0}^n(a_kx_i^k)^2+\sum_{j=0}^n\sum_{\substack{k=0\\k \neq j}}^na_ja_kx_i^{j+k}\right)\\
  \nonumber \dfrac{\partial B}{\partial a_j} & = &  2\sum_{k=0}^n\left(a_k\sum_{i=0}^Mx_i^{j+k}\right)\\
  \nonumber 0 = \dfrac{\partial E}{\partial a_j} & = & -2\sum_{i=0}^My_ix_i^j + 
  2\sum_{k=0}^n\left(a_k\sum_{i=0}^Mx_i^{j+k}\right) \forall j=0,\ldots,n
\end{eqnarray}
\end{itemize}} 
%%%%
\frame{
\frametitle{M\'inimos Cuadrados}
\begin{itemize}
  \item<1->De esta manera las ecuaciones normales vienen definidas por 
  $$
  EN \equiv \left\{\sum_{k=0}^n\left(a_k\sum_{i=0}^Mx_i^{j+k}\right) = \sum_{i=0}^My_ix_i^j \qquad \forall j=0,\ldots,n 
  \right.
  $$
  \item<2->Generando as\'i el siguiente sistema de ecuaciones lineales
  \item   
  $$
  \left\{\begin{array}{l}
          a_0(M+1) + a_1\displaystyle\sum_{i=0}^Mx_i^1+\cdots+a_n\displaystyle\sum_{i=0}^Mx_i^n =\displaystyle 
  \sum_{i=0}^My_i\\
          a_0\displaystyle\sum_{i=0}^Mx_i^1 + 
  a_1\displaystyle\sum_{i=0}^Mx_i^2+\cdots+a_n\displaystyle\sum_{i=0}^Mx_i^{n+1} = \displaystyle\sum_{i=0}^My_ix_i\\
          \vdots\\
          a_0\displaystyle\sum_{i=0}^Mx_i^n + 
  a_1\displaystyle\sum_{i=0}^Mx_i^{n+1}+\cdots+a_n\displaystyle\sum_{i=0}^Mx_i^{n+n} = \displaystyle\sum_{i=0}^My_ix_i^n\\
         \end{array}
  \right.
  $$  
\end{itemize}}
%%%%
\frame{
\frametitle{Aproximaci\'on de Exponencial de M\'inimos Cuadrados}
\begin{itemize}
  \item<1-> En este caso la funci\'on aproximante es:
  $$
  y = be^{ax} \qquad \mbox{ o } \qquad y=bx^a
  $$
  \item<2-> En general para cualquier funci\'on aproximante:
  $$
  0 \leq E =\sum_{i=0}^M(y_i - y(x_i))^2 \quad \mbox{ fijando } x_0,x_1,\ldots,x_M
  $$
  \item<3-> Si $\exists \dfrac{\partial y}{\partial j}\forall j=0,\ldots,n$ y son continuas $ \Rightarrow \exists \min E$.
  \end{itemize}
}
%%%%
\frame{
\frametitle{Aproximaci\'on de Exponencial de M\'inimos Cuadrados}  
\begin{itemize}
  \item<1-> Para el caso de las ecuaciones normales para $y=be^{ax}$.  
  $$
  E = \sum_{i=0}^M\left(y_i - be^{ax_i}\right)^2 = \sum_{i=0}^My_i^2-2b\sum_{i=0}^M\left(yie^{ax_i}\right) + 
  b^2\sum_{i=0}^M\left(e^{2ax_i}\right)
  $$   
  $$
  \left\{\begin{array}{l}
          \dfrac{\partial E}{\partial a}=0\\
          \dfrac{\partial E}{\partial b}=0\\
         \end{array}\right. \Rightarrow \left\{
         \begin{array}{lcl}
          -2b\displaystyle\sum_{i=0}^M\left(y_ix_ie^{ax_i}\right) + 2b^2\displaystyle\sum_{i=0}^M\left(x_ie^{2ax_i}\right) 
  & = & 0\\
          -2\displaystyle\sum_{i=0}^M\left(y_ie^{ax_i}\right) + 2b\displaystyle\sum_{i=0}^M\left(e^{2ax_i}\right) & = & 
  0\\
         \end{array}\right.
  $$  
  $$
  \left\{\begin{array}{l}
          b\displaystyle\sum_{i=0}^M\left(x_ie^{2ax_i}\right) - \displaystyle\sum_{i=0}^M\left(y_ix_ie^{ax_i}\right) =0\\
          b\displaystyle\sum_{i=0}^M\left(e^{2ax_i}\right) - \displaystyle\sum_{i=0}^M\left(y_ie^{ax_i}\right) =0
         \end{array}\right.  
  \left\{\begin{array}{l}
          \displaystyle\sum_{i=0}^M\left(y_ix_ie^{ax_i}\right) =b\displaystyle\sum_{i=0}^M\left(x_ie^{2ax_i}\right)\\
           \displaystyle\sum_{i=0}^M\left(y_ie^{ax_i}\right) =b\displaystyle\sum_{i=0}^M\left(e^{2ax_i}\right)
         \end{array}\right.
         $$
         {\rightarrowfill Sistema de Ecuaciones No Lineales}
    \end{itemize}
}
%%%%
\frame{
\frametitle{Aproximaci\'on de Exponencial de M\'inimos Cuadrados}
\begin{itemize}
  \item<1-> Suponiendo $y=be^{ax}>0$ y $b>0$
  $$
  \ln(y) = \ln(b) + \ln(e^{ax}) \Rightarrow \ln(y) = \ln(b)+ax
  $$
  se trabaja como si el problema fuera lineal, de este modo se obtiene:
  \begin{eqnarray}
  \nonumber \ln(b) & = & \dfrac{\displaystyle\sum_{i=0}^Mx_i\displaystyle\sum_{i=0}^M\ln(y_i)x_i
  -\displaystyle\sum_{i=0}^M\ln(y_i)\displaystyle\sum_{i=0}^Mx_i^2 }{\left(\displaystyle\sum_{i=0}^Mx_i\right)^2
  -(M+1)\displaystyle\sum_{i=0}^Mx_i^2}\\
  \nonumber a & = & 
  \dfrac{\displaystyle\sum_{i=0}^Mx_i\displaystyle\sum_{i=0}^M\ln(y_i) -(M+1)\displaystyle\sum_{i=0}^M\ln(y_i)x_i}{
  \left(\displaystyle\sum_{i=0}^Mx_i\right)^2
  -(M+1)\displaystyle\sum_{i=0}^Mx_i^2}
  \end{eqnarray}
\end{itemize}  
}

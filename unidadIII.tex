...
Por ser $\lambda$ un autovalor de $A$ y sea $x$ el autovector asociado, se
cumple que
$$
Ax = \lambda x \Rightarrow x = A^{-1}\lambda x \Rightarrow \lambda^{-1}x = A^{-1}x
$$
por lo tanto $\lambda^{-1}$ es autovalor de $A^{-1}$.}
\end{frame}
%%%%%
\begin{frame}{M\'etodo de la Potencia Inversa}
  \begin{itemize}
  ...
  \begin{frame}{M\'etodo de la Potencia Trasladada}
    \begin{itemize}
      ...
      \uncover<2->{
      \begin{block}{Teorema:}
        Si $\lambda$ es autovalor de una matriz $A$ asociado a un autovector $v$ y $\alpha$ una
     constante cualquiera se verifica que $\lambda-\alpha$ es un autovalor de $A - \alpha I$ asociado 
     al mismo autovector $v$.
      \end{block}}
    \end{itemize} 
    \uncover<3->{{\textbf Demostraci\'on}:

    Se sabe por hip\'otesis que $Av = \lambda v$, por lo tanto
    \begin{eqnarray}
       Av = \lambda v &\Rightarrow& Av-\alpha v = \lambda v - \alpha v \nonumber\\
      & \Rightarrow& (A-\alpha I)v = (\lambda-\alpha)v\nonumber
    \end{eqnarray}}       
  \end{frame}
  %%%%%%
  \begin{frame}{M\'etodo de la Potencia Trasladada}
...
BSx = \lambda Sx
$$
Puesto que $x \neq 0$ y $S$ es no singular $Sx \neq 0$.Por tanto, $Sx$ es un autovector de $B$
...
\uncover<2->{
\begin{block}{}
 {\bf Inicio}: $A^0 = A = QR \mbox{ (n\'otese que $R=Q^tA$)}$

$A^{1} = RQ = Q^tAQ \mbox{ ($A$ y $A^1$ son similares)}$
 
\end{block}} 
 \uncover<3->{
\begin{block}{}
 
{\bf Factorizar:} $A^1 = Q^1R^1$

$A^2 = R^1Q^1 = Q^{1t}Q^tAQQ^1 \mbox{ (similar a $A^1$ y $A$)}$
...
{\bf Factorizar}: $A^m = Q^mR^m$

$A^{m+1} = R^mQ^m (\mbox{ (similar a $A^m,\ldots,A^1,A$)}$
...
\uncover<5->{
Si $A^m$ llegar\'a a ser diagonal, los autovectores de $A^m$ son $e_1$, $e_2$, $\ldots$, $e_n$. 
Dado que $A^m$ y $A$ son matrices similares, los autovectores de $A$ son $(Q^{*t})^{-1}e_i=Q^*$, es 
decir {\bf los autovectores de  $A$ son las columnas de $Q^*$}.}

\uncover<6->{
\begin{block}{}
   Si $A$ es una matriz sim\'etrica ($A=A^t$),entonces los autovectores de $A$ son las columnas de 
$Q^*$.  
\end{block}
}
